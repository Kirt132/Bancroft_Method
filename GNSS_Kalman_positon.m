function [est_r_ea_e,est_clock, num_iter,GDOP] = GNSS_Kalman_position(...
    GNSS_measurements,no_GNSS_meas,predicted_X,W,Q_matrix)
...
while test_convergence>0.0001
    ...
    % Weighted Least of squares with priori (9.35)/(9.141)
    K = Q_matrix*H_matrix(1:no_GNSS_meas,:)'*inv(H_matrix(1:no_GNSS_meas,:)*Q_matrix*H_matrix(1:no_GNSS_meas,:)'+inv(W));
    x_est = x_pred + K *(GNSS_measurements(1:no_GNSS_meas,1) -  pred_meas(1:no_GNSS_meas));
    %x_est = x_pred + inv(H_matrix(1:no_GNSS_meas,:)'*W*H_matrix(1:no_GNSS_meas,:)+inv(Q_matrix))*(H_matrix(1:no_GNSS_meas,:)'*W*(GNSS_measurements(1:no_GNSS_meas,1) -  pred_meas(1:no_GNSS_meas))+inv(Q_matrix)*predicted_X);
    % Test convergence    
